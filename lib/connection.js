...
Connection.prototype._chooseReachableHost = function(max, timeout, tries) {
  tries = tries || 1;
  if(tries > max)
    return null;
  var hostName = this._chooseHost();
  var res = connectionTester.test(hostName, this.options.port, timeout);
  if(res.success)
    return hostName;
  else
    return this._chooseReachableHost(max, timeout, ++tries);
};

Connection.prototype._createSocket = function() {
  var hostName = this._chooseReachableHost(this.options.host.length * 2, 500), self = this, port = this.options.port;
  if(!hostName) {
    throw new Error('Unable to find reachable host.');
  }
